...
class WordTrigger(Trigger):
	...
	def isWordIn(self, text):
		return self.word in re.findall(r"[\w]+", text.lower())
...
class PhraseTrigger(Trigger):
	...
	def evaluate(self, story):
		...
		summary1 = story.getSummary()
		return self.phrase in title1 or self.phrase in subject1 or self.phrase in summary1
...
def filterStories(stories, triggerlist):
	...
	for story in stories:
		for trigger in triggerlist:
			if (trigger.evaluate(story) and story not in stories1):
				stories1.append(story)
	return stories1
...
def readTriggerConfig(filename):
    ...
    all = [ line.rstrip() for line in triggerfile.readlines() ] #the blank line converted to empty string
    lines = []
    for line in all:
...
if __name__ == '__main__':
    ...
    thread.start_new_thread(main_thread, (root,)) # in the window created by the root, run main_thread method
    root.mainloop()
